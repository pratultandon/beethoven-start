...
def rnnrbm():
    ...
    def generate(num, x=x, size_bt=size_bt, u0=u0, n_visible=n_visible, prime_length=100):
        ...
        [_, _, _, _, _, music] = tf.while_loop(lambda count, num_iter, *args: count < num_iter,
                                                         generate_recurrence, [count, k, U, primer, x, music],
																												 # TODO: put correct dims in shape_invariants
																												 shape_invariants=[count.get_shape(), k.get_shape(), U.get_shape(), primer.get_shape(), x.get_shape(), tf.TensorShape([None, n_visible])], parallel_iterations=1)
        return music
